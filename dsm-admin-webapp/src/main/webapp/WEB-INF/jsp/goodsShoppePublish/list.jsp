...
<div id="content">
	<div id="content-header">
		<div id="breadcrumb"> 
		  ...
		  <a href="#" class="tip-bottom">专柜列表</a> 
		  <a href="#" class="current">专柜商品</a> 
	  </div>
	</div>
...
function getSelectionData(offset) {
	$.ajax({
		type : 'POST',
		dataType : 'json',
		data : {"shoppeId":shoppeId,"offset":offset,"limit":limit},
		url : '${basepath}/goodsShoppePublish/list',
...
function callback(records){
	return records.map(function(ele, index){
		return getHtml(ele.idx, ele.goodsName, ele.images);
	}).join("");
}
	...
	$table = $('#tableBox').bootstrapTable({
        ...
        columns: [{
            ...
            formatter: function(data, row ,index){
                ...
                html += ' <a class="btn btn-primary btn-sm btn-mini">加入</a>';
                html += '</div>';
                return html;
            }
        }],
...
$.extend({
	select: function(_id, _goodsName, _images){
		W.$.confirm(
				"确定要加入专柜吗？",function(){
		var AjaxURL = "${basepath}/goodsShoppePublish/saveOrUpdate"; //API接口
		$.ajax({
			type: "POST",
			dataType: "json",
			url: AjaxURL,
	...
	cancel: function(_id){
		W.$.confirm(
				"确定要移除专柜吗？",function(){
		var AjaxURL = "${basepath}/goodsShoppePublish/batchDelete"; //API接口
		$.ajax({
			type: "POST",
			dataType: "json",
			url: AjaxURL,
